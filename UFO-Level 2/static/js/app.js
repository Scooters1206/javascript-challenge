...
function runEnter() {
    ...
    var searchterm = d3.select("browser");

    // If the selection is equal to Date/Time
    ...
    if (searchterm == "City") {
            ...
            let input = d3.select("#datetime")
            // Get the value property of the input element
            let cityvalue = input.property("value")
            // Use the form input to filter the data by date
            let newData = tableData.filter(item => item.city === cityvalue)
            console.log(newData)
